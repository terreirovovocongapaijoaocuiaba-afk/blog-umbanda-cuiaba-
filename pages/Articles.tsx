    ...
    <div className="pt-32 pb-20 container mx-auto px-6 animate-fadeIn">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          {articles.length > 0 && (
            <Link to={`/artigos/${articles[0].id}`} className="lg:col-span-2 group cursor-pointer relative overflow-hidden rounded-2xl h-[500px] border border-stone-800 block">
              <img 
              ...
              <div className="absolute bottom-0 left-0 p-8 md:p-12">
                <span className="bg-umbanda-red text-white px-3 py-1 text-xs font-bold uppercase rounded mb-4 inline-block">
                  Destaque
                </span>
                <h2 className="text-3xl md:text-5xl font-serif font-bold text-white mb-4 group-hover:text-umbanda-gold transition-colors">
                  {articles[0].title}
                </h2>
                <p className="text-stone-300 max-w-2xl mb-6 hidden md:block">
                  {articles[0].excerpt}
                </p>
          ...
          {articles.slice(1).map(article => (
            <Link to={`/artigos/${article.id}`} key={article.id} className="flex flex-col md:flex-row gap-6 group cursor-pointer bg-stone-900/40 p-4 rounded-xl hover:bg-stone-900 transition-colors border border-transparent hover:border-stone-800">
              ...
              <div className="flex flex-col justify-center">
                ...
                <h3 className="text-xl font-serif font-bold text-stone-100 mb-2 group-hover:text-umbanda-red transition-colors">
                  {article.title}
                </h3>
